...
OCtrlPts = zeros(size(ICtrlPts, 1), NCtrlPts + NInsKnts);
OKntVect = zeros(1, numel(IKntVect) + NInsKnts);
a = FindSpan(NCtrlPts, p, InsKnts(1), IKntVect);
b = FindSpan(NCtrlPts, p, InsKnts(NInsKnts), IKntVect);
OCtrlPts(:, 1 : a - p) = ICtrlPts(:, 1 : a - p);

OCtrlPts(:, b + NInsKnts : NCtrlPts + NInsKnts) = ICtrlPts(:, b : NCtrlPts);

OKntVect(1 : a) = IKntVect(1 : a);

OKntVect(b + p + 1 + NInsKnts : numel(IKntVect) + NInsKnts) = IKntVect(b + p + 1 : numel(IKntVect));
...
for j = NInsKnts : -1 : 1
    ...
    for l = 1 : p
        ...
        if abs(alfa) == 0
            ...
            alfa = alfa / (OKntVect(k + l) - IKntVect(i - p + l));
            OCtrlPts(:, ind - 1) = alfa * OCtrlPts(:, ind - 1) + (1 - alfa) * OCtrlPts(:, ind);
        end
    end
